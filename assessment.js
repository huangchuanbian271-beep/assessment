...
assessmentButton.addEventListener(//イベント検知設定の追加
  'click', //クリックイベント
   () => { //無名関数でアロー関数、イベント検知したら実行される
      const userName = userNameInput.value;//入力欄（input）の
    ...
    //診断結果表示エリアの作成
    resultDvision.innerText = '';//divタグを空文字で上書きすることで、空にしている。
    const header =document.createElement('h3')//h3タグの作成
    header.innerText = '診断結果'//タグの内側のテキストを設定
    resultDvision.appendChild(header)//divタグの子要素として追加
    ...
    resultDvision.appendChild(paragraph)//divタグの子要素としてpタグを追加
